...
export default async function AdminMentorsPage() {
  ...
  const [{ data: nomData }, { data: mentorData }] = await Promise.all([
    admin
      ...
      .select(
        "id, member_id, pod_id, nominated_by, status, decision_reason, decided_at, created_at",
      )
      .order("created_at", { ascending: false }),
    admin
      .from("mentor_profiles")
      .select("member_id, mentor_status, capability_pod_id, verified_at"),
  ]);
  ...
  const [{ data: profileRows }, { data: memberRows }, { data: podRows }] =
    await Promise.all([
      memberIds.length
        ? admin
            .from("profiles")
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="text-eten-faint mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
                    ...
                    <span>Pod: {podById.get(n.pod_id) ?? "—"}</span>
                    <span>
                      Nominated by{" "}
                      {n.nominated_by
